...
“เห็นหลักฐานและรู้ว่าจะตรวจอะไรต่อ” ได้ใน workspace เดียว โดยต่อยอดระบบ
client-go, watch/cache, Argo CD และ GitOps ที่มีอยู่

ลำดับความสำคัญ:

1. เปิด resource ที่ผู้ใช้มีสิทธิ์เข้าถึงได้ และควบคุมการเขียนจากแอปได้ชัดเจน
2. เพิ่ม Explain Why และ filter ตามสถานะ/ปริมาณ เพื่อค้นและวิเคราะห์ incident
3. เพิ่มประวัติที่แอปสังเกตเห็น และ logs รวมตาม workload
4. เพิ่ม Helm inspector และ workflow extensions ตามการใช้งานจริง
...
**IW-03: Evidence-based incident report**

- สร้าง read-only evidence collector โดย reuse resource inspection,
  UID ownership traversal และ Events readers
...
- เพิ่ม Explain ใน resource inspector/detail; render partial report ได้
- ห้ามส่ง logs, Secrets, kubeconfig หรือ report ไป external AI/service
...
Exit: incident scenarios ให้คำอธิบายที่ตามกลับถึงหลักฐานได้;
filter ไม่ให้ผลผิดจาก pagination, units, metric age หรือ stale query events

**Release A: recommended first release**

Phase 0–2 คือขอบเขตรอบแรก: discovery fallback + read-only + Explain + typed
filters รวม selector controls หลัง watch identity tests ผ่าน
ไม่ผูก release นี้กับ timeline, Helm, Flux หรือ generic bulk
