...
- เข้าใจว่า workload/resource ไม่พร้อมเพราะอะไร พร้อมหลักฐานที่ตรวจสอบย้อนกลับได้
  และรู้ว่าจะตรวจอะไรต่อ โดยไม่ต้องประกอบจากหลายหน้าด้วยตนเอง
...
- **EXP-04:** แต่ละ finding มีอาการ, severity, summary, supporting evidence,
  suggested next steps และ `confidenceClass` (`confirmed`, `supported`, `unknown`);
  แยก observed fact จาก possible cause เช่น OOMKilled ยืนยัน termination reason
  แต่ยังยืนยัน memory leak ไม่ได้ `confirmed` ใช้เมื่อหลักฐานยืนยัน finding โดยตรง,
  `supported` ใช้เมื่อหลักฐานรองรับสาเหตุแต่ยังไม่ยืนยันครบ และ `unknown` ใช้เมื่อ
  หลักฐานไม่พอ ห้ามสร้างเปอร์เซ็นต์ confidence ที่ไม่มี measurement model รองรับ
- **EXP-05:** ไม่ใช้สีเขียวหรือ “Healthy” จากการไม่มี evidence; ถ้าอ่าน Pods/Events
  ไม่ได้ให้ partial/unknown และอธิบายส่วนที่ขาด แม้ root GET สำเร็จ missing evidence
  ไม่ได้แปลว่าไม่มีปัญหา และ rule ต้องไม่ infer conclusion เกิน collected evidence
...
Finding: stable ruleID, severity, summary, confidenceClass, observedFacts[],
possibleCauses[], evidenceIDs[], nextSteps[]. Evidence: sourceRef+UID,
fieldPath/eventUID,
...
- หลักฐานไม่พอให้ `confidenceClass=unknown` / insufficient evidence แทนการเดา
